...
/*C        COMPUTE CAPITAL  U, CAPITAL V, Z AND H*/
    ...
    // Loop fision
    // First, the part that can be vectorized and its worthy
    // Second, the problems of the vect
...
/*C     PERIODIC CONTINUATION*/
	...
	#pragma omp for simd nowait schedule(simd:static)
    for(J=0;J<N;J++){
        CU[0][J] = CU[M][J];
        CV[M][J+1] = CV[0][J+1];
        Z[0][J+1] = Z[M][J+1];
...
/*C        COMPUTE NEW VALUES OF U,V,P*/
    ...
    // Vectorize usig loop fision here is unworthy
	#pragma omp parallel private(I,J)
	{
	#pragma omp for schedule(static)
    for(I=0;I<M;I++){
